...
test(`${ENTITY}: stored javascript: and data: URLs render as inert text, never as links`, async () => {
  const jsId = await seedWith(stack, ENTITY, { "contentUrl": 'javascript:alert(1)' }, jar);
  ...
  const dataId = await seedWith(stack, ENTITY, { "contentUrl": 'data:text/html,x' }, jar);
...
test(`${ENTITY}: stored http(s) URL renders as a clickable link`, async () => {
  const id = await seedWith(stack, ENTITY, { "contentUrl": 'https://example.com/profile' }, jar);
